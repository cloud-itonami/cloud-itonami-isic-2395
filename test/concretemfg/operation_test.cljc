(ns concretemfg.operation-test
  "Smoke tests for the compiled ConcreteOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `concretemfg.governor-contract-test`; the Store contract in
  `concretemfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "ConcreteOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t1"
                          {:op :log-production-batch :effect :propose :subject "batch-001"
                           :patch {:product-type :concrete-pipe}}
                          coordinator)
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "batcher-001" :severity :moderate :description "curing chamber steam-burn risk"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))

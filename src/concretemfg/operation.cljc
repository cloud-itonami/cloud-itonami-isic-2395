(ns concretemfg.operation
  "ConcreteOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (ConcreteAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Concrete Plant
  ...
  (:require [langgraph.graph :as g]
            ...
            [concretemfg.advisor :as advisor]
...
(defn build
  "Compiles a ConcreteOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `concretemfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ConcreteAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Concrete Plant Operations Governor -- independent censor.
      (g/add-node :govern

(ns concretemfg.advisor
  "ConcreteAdvisor -- the *contained intelligence node* for the
  concrete/cement/plaster products plant-operations coordination
  actor.

  It normalizes production-batch patches (product-type/weight/
  dimensional-deviation/defect-rate), drafts a mixing/batching-plant
  or molding-line-equipment maintenance scheduling proposal against a
  piece of equipment, drafts a materials-safety/equipment-safety
  concern flag, and drafts an outbound concrete/cement/plaster product
  ...
  a real mixing/molding-line actuation or freight dispatch. Every
  output is censored downstream by `concretemfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `concretemfg.governor`)
  ...
  contract every caller of this actor agrees to) -- `concretemfg.governor`
...
(defn- log-production-batch
  "Production-batch (cured lot) intake upsert -- the advisor only
...
(defn- schedule-maintenance
  "Draft a mixing/batching-plant or molding-line-equipment
  maintenance-window scheduling proposal against a piece of
  equipment. The advisor reports what it can see (equipment
  verified?/registered?) in its rationale, but `concretemfg.governor`
  NEVER trusts this report -- it independently re-derives
  verified?/registered? from the equipment's own stored fields before
    ...
    {:summary    (str subject " 向け保守作業予定提案 (" (:maintenance-type value) ")"
     ...
     :rationale  (if eq
                   (str "equipment-verified?=" (registry/equipment-verified? eq)
                        ...
                        " actuate-mixing-line?=" (boolean (:actuate-mixing-line? value)))
                   (str equipment-id " が見つかりません"))
     :cites      (if eq [equipment-id] [])
     :effect     :maintenance/schedule
     :value      value
     :stake      nil
     :confidence (if (and ready? (not (:actuate-mixing-line? value))) 0.9 0.3)}))

(defn- flag-safety-concern
  "Draft a materials-safety/equipment-safety concern -- cement/silica-
  dust hazard (batching/mold-stripping dust exposure), curing-heat
  hazard (steam curing/autoclave burn or scald exposure), or
  mixing/batching-plant pinch-point/equipment-safety concern. ALWAYS
  ...
  `concretemfg.phase`: no phase ever adds this op to a phase's `:auto`
  set; `concretemfg.governor` also always escalates on `:coordination/
...
(def ^:private system-prompt
  (str "あなたはコンクリート・セメント・石膏製品工場プラント運用"
       "コーディネーターの助言者です。"
       ...
       "ミキサー・バッチングプラント・成形ライン等の設備の直接操作(actuate)を絶対に提案してはいけません"
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `concretemfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :concretemfg-advisor-proposal
   :op         (:op request)

(ns concretemfg.governor
  "Concrete Plant Operations Governor -- the independent compliance
  layer that earns the ConcreteAdvisor the right to commit. The
  ...
  mixing/batching plant or molding line, whether a shipment proposal's
  own claimed weight would blow through the batch's own logged
  production weight, or when an act stops being a coordination
  proposal and becomes direct mixing/molding-line-equipment control,
  so this MUST be a separate system able to *reject* a proposal and
  fall back to HOLD.

  `:itonami.blueprint/governor` is `:concrete-plant-operations-governor`
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:molding-line/
                                       actuate` or `:batching-plant/
                                       run`) is the 'direct mixing/
                                       molding-line-equipment control'
                                       scope violation this actor must
                                       NEVER perform -- HARD,
                                       PERMANENT, unconditional.
    4. Mixing-line-actuate blocked -- for `:schedule-maintenance`, does
                                       the proposal's own `:value`
                                       declare `:actuate-mixing-line?
                                       true`? Directly actuating the
                                       mixing/batching plant or molding
    ...
    5. Equipment not verified/
       registered                  -- for `:schedule-maintenance`,
                                       ...
                                       are both true
                                       (`concretemfg.registry/equipment-
   ...
   10. Invalid dimensional-
       deviation                    -- for `:log-production-batch`, if
                                       ...
                                       plausible reading
                                       (`concretemfg.registry/dimensional-
...
(def allowed-proposal-effects
  ...
  molding-line-equipment-control effect."
...
(defn- mixing-line-control-blocked-violations
  ...
  Anything else (direct mixing/molding-line control, a fabricated
  actuation effect) is this actor's central scope boundary."
  [proposal]
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :mixing-line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") はミキサー・バッチングプラント・成形ライン等の直接操作に該当する可能性があり、恒久的に禁止")}]))

(defn- mixing-line-actuate-blocked-violations
  "HARD, PERMANENT, unconditional: a `:schedule-maintenance` proposal
  whose own `:value` declares `:actuate-mixing-line? true` is
  attempting to directly actuate the mixing/batching plant or molding
  line -- this actor may only ever propose/schedule a DRAFT
  maintenance window, never actuate the mixing/molding line directly.
  No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-mixing-line? (:value proposal))))
    [{:rule :mixing-line-actuate-blocked
      :detail "ミキサー・バッチングプラント・成形ライン等の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn check
  "Censors a ConcreteAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (mixing-line-control-blocked-violations proposal)
                           (mixing-line-actuate-blocked-violations request proposal)
